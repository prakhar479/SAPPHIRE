    ...
    def load_checkpoint(self, stage: str) -> Optional[Any]:
        ...
        try:
            ...
            if metadata_file.exists():
                ...
                logger.info(f"Loaded checkpoint for stage '{stage}' from {metadata['timestamp']}")
...
class MemoryManager:
    ...
    def force_garbage_collection(self):
        ...
        gc.collect()
        self.logger.debug(f"Garbage collection performed. Memory usage: {self.get_memory_usage():.2f} GB")
...
class EnhancedProcessingPipeline:
    ...
    def process_track_chunk(self, track_chunk: List[MusicTrack], chunk_id: int) -> List[Dict[str, Any]]:
    ...
    def parallel_process_chunks(self, track_chunks: List[List[MusicTrack]]) -> List[Dict[str, Any]]:
    ...
    def load_datasets_with_optimization(self, datasets: Optional[List[str]] = None) -> List[MusicTrack]:
        """Load datasets with memory optimization."""
        stage_name = "dataset_loading"
        
        # Check for existing checkpoint
        if (self.config.optimization['resume_from_checkpoint'] and 
            self.checkpoint_manager.checkpoint_exists(stage_name)):
    ...
    def extract_features_optimized(self, tracks: List[MusicTrack]) -> pd.DataFrame:
        ...
        self.logger.info(f"Starting optimized feature extraction for {len(tracks)} tracks...")
        ...
        # Update statistics
        self.stats['features_extracted'] = len(features_df.columns) - 4  # Exclude metadata columns
    ...
    def preprocess_with_advanced_filtering(self, tracks: List[MusicTrack]) -> List[MusicTrack]:
        ...
        for i in range(0, len(tracks), batch_size):
            ...
            if self.config.processing.progress_bar:
                print(f"Preprocessing batch {i//batch_size + 1}/{(len(tracks) + batch_size - 1)//batch_size}")
    ...
    def run_optimized_pipeline(self, 
                             ...
                             limit_tracks: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            final_stats = {
                **self.stats,
                ...
                'success_rate': self.stats['tracks_processed'] / len(tracks) if tracks else 0
            }
    ...
    def get_processing_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'checkpoints': {
                'dataset_loading': self.checkpoint_manager.checkpoint_exists('dataset_loading'),
                'preprocessing': self.checkpoint_manager.checkpoint_exists('preprocessing'),
                'feature_extraction': self.checkpoint_manager.checkpoint_exists('feature_extraction')
            }
        }
