...
class Pipeline:
    ...
    def run_full_pipeline(self, datasets: List[str] = None, output_dir: str = None, 
                         use_enhanced_processing: bool = True, limit_tracks: int = None) -> Dict[str, Any]:
        """
        ...
        try:
            if use_enhanced_processing:
                ...
                self.logger.info("Using enhanced processing pipeline with optimization...")
            ...
            self.analysis_results = self.analyze_features(self.features_df, str(output_path))
            
            # Step 5: Train mood classifiers
            self.logger.info("Step 5: Training mood classifiers...")
            self.model_results = self.train_classifiers(self.features_df, str(output_path))
            ...
            return {
                ...
                'tracks_processed': len(self.features_df) if self.features_df is not None else 0,
                'features_extracted': len(self.features_df.columns) if self.features_df is not None else 0,
                'output_directory': str(output_path),
                'report': report,
                'processing_stats': processing_result.get('statistics', {}) if use_enhanced_processing else {}
            }
    ...
    def preprocess_data(self, tracks: List[MusicTrack]) -> List[MusicTrack]:
        ...
        self.logger.info(f"Preprocessed {len(processed_tracks)} tracks (filtered from {len(tracks)})")
    ...
    def extract_features(self, tracks: List[MusicTrack]) -> pd.DataFrame:
        ...
        for track in tracks:
            try:
                ...
                self.logger.error(f"Error extracting features for track {track.track_id}: {e}")
                continue
        ...
        self.logger.info(f"Extracted {len(features_df.columns)} features for {len(features_df)} tracks")
        return features_df
    ...
    def analyze_features(self, features_df: pd.DataFrame, output_dir: str) -> Dict[str, Any]:
        ...
        analysis_results['dataset_analysis'] = self.analyzer.analyze_dataset(features_df, output_dir)
        
        # Cross-modal analysis (key component of SAPPHIRE)
        analysis_results['cross_modal_analysis'] = self.mood_classifier.cross_modal_analysis(features_df)
        
        # Feature clustering
        analysis_results['clustering'] = self.analyzer.perform_clustering(features_df, output_dir)
    ...
    def create_visualizations(self, output_dir: str):
        ...
        if self.model_results and self.mood_classifier.best_model:
            ...
            if not importance_df.empty:
                self.visualizer.create_feature_importance_plot(importance_df, str(viz_dir))
    ...
    def generate_report(self, output_dir: str) -> Dict[str, Any]:
        ...
        report = {
            ...
            'data_summary': {
                ...
                'processed_tracks': len(self.features_df) if self.features_df is not None else 0,
                'total_features': len(self.features_df.columns) if self.features_df is not None else 0,
                'datasets': self.data_loader.get_dataset_statistics(self.tracks) if self.tracks else {}
            },
            'analysis_summary': {
                'cross_modal_analysis': {
                    'mean_jaccard_similarity': self.analysis_results.get('cross_modal_analysis', {}).get('jaccard_similarity', {}).get('mean', 0),
                    'max_cross_correlation': self.analysis_results.get('cross_modal_analysis', {}).get('correlations', {}).get('max_correlation', 0)
                } if 'cross_modal_analysis' in self.analysis_results else {},
                'clustering_results': self.analysis_results.get('clustering', {})
            },
            'model_performance': {
                'best_model': max(self.model_results.keys(), key=lambda k: self.model_results[k]['test_accuracy']) if self.model_results else None,
                'best_accuracy': max([r['test_accuracy'] for r in self.model_results.values()]) if self.model_results else 0,
                'model_comparison': {name: result['test_accuracy'] for name, result in self.model_results.items()}
        ...
        result = {
            ...
            'all_probabilities': {
                label: float(prob) for label, prob in zip(
                    self.mood_classifier.label_encoder.classes_, 
                    probabilities[0]
                )
