...
class Preprocessor:
    ...
    def preprocess_track(self, track: MusicTrack) -> Tuple[MusicTrack, Dict]:
        ...
        try:
            ...
            quality_metrics['passes_quality_check'] = self._assess_overall_quality(quality_metrics)
    ...
    def _preprocess_audio(self, track: MusicTrack) -> Tuple[MusicTrack, Dict]:
        ...
        try:
            ...
            quality_metrics.update(self._assess_audio_quality(track.audio_data, track.sample_rate))
            
            # Normalize audio
            track.audio_data = self._normalize_audio(track.audio_data, track.sample_rate)
    ...
    def _preprocess_lyrics(self, track: MusicTrack) -> Tuple[MusicTrack, Dict]:
        ...
        try:
            ...
            quality_metrics['completeness'] = self._assess_lyrics_completeness(track.lyrics_text)
    ...
    def _assess_audio_quality(self, audio_data: np.ndarray, sample_rate: int) -> Dict:
        ...
        try:
            ...
            if self.loudness_meter:
                try:
                    metrics['integrated_loudness_lufs'] = self.loudness_meter.integrated_loudness(audio_data)
                except:
            ...
            # Quality flags
            metrics['snr_ok'] = metrics['snr_db'] >= self.config.quality_thresholds['snr_min']
            metrics['clipping_ok'] = metrics['clipping_percentage'] < 5.0  # Less than 5% clipping
            metrics['vocal_ok'] = metrics['vocal_dominance_score'] >= self.config.quality_thresholds['vocal_dominance_min']
        ...
        try:
            if self.loudness_meter and HAS_PYLOUDNORM:
                ...
                normalized_audio = pyln.normalize.loudness(
                    audio_data, 
                    current_loudness, 
                    self.config.audio.target_loudness
                )
    ...
    def _assess_lyrics_completeness(self, text: str) -> float:
        ...
        has_incomplete_markers = any(marker in text.lower() for marker in incomplete_markers)
    ...
    def _assess_overall_quality(self, quality_metrics: Dict) -> bool:
        ...
        if audio_quality:
            audio_ok = (
                audio_quality.get('duration_ok', False) and
                audio_quality.get('snr_ok', False) and
                audio_quality.get('clipping_ok', True) and
                audio_quality.get('vocal_ok', False)
            )
        ...
        if lyrics_quality:
            ...
            lyrics_ok = completeness >= self.config.quality_thresholds['lyrics_completeness_min']
        
        # Overall assessment
        return audio_ok and lyrics_ok
    
    def preprocess_batch(self, tracks: List[MusicTrack], n_workers: int = None) -> Tuple[List[MusicTrack], pd.DataFrame]:
        """
        ...
        if n_workers == 1:
            ...
            results = [self.preprocess_track(track) for track in tqdm(tracks, desc="Preprocessing tracks")]
        else:
            # Parallel processing
            with ProcessPoolExecutor(max_workers=n_workers) as executor:
                futures = [executor.submit(self.preprocess_track, track) for track in tracks]
                results = []
    ...
    def filter_high_quality_tracks(self, tracks: List[MusicTrack], quality_df: pd.DataFrame) -> List[MusicTrack]:
        ...
        self.logger.info(f"Filtered {len(high_quality_tracks)} high-quality tracks from {len(tracks)} total")
        ...
        if 'audio_quality' in quality_df.columns:
            ...
            if audio_metrics:
                ...
                report['audio_quality'] = {
                    'mean_snr_db': audio_df['snr_db'].mean() if 'snr_db' in audio_df else None,
                    'mean_duration': audio_df['duration'].mean() if 'duration' in audio_df else None,
                    'clipping_rate': (audio_df['clipping_percentage'] > 5.0).mean() if 'clipping_percentage' in audio_df else None
                }
        ...
        if 'lyrics_quality' in quality_df.columns:
            ...
            if lyrics_metrics:
                ...
                report['lyrics_quality'] = {
                    'mean_completeness': lyrics_df['completeness'].mean() if 'completeness' in lyrics_df else None,
                    'mean_word_count': lyrics_df['word_count'].mean() if 'word_count' in lyrics_df else None
                }
...
def advanced_quality_assessment(self, track: MusicTrack) -> Dict[str, float]:
        ...
        if len(audio) < sr * 0.1:  # Less than 0.1 seconds
            self.logger.debug(f"Very short audio clip: {len(audio) / sr:.3f}s for track {track.track_id}")
        ...
        try:
            ...
            # Silence detection
            silence_frames = np.sum(magnitude < self.config.quality_thresholds['silence_threshold'])
            total_frames = magnitude.shape[1]
            quality_metrics['silence_ratio'] = silence_frames / total_frames if total_frames > 0 else 1.0
            ...
            quality_metrics['estimated_snr_db'] = 10 * np.log10(signal_power / (noise_power + 1e-10))
            ...
            vocal_mask = (freq_bins >= vocal_freq_range[0]) & (freq_bins <= vocal_freq_range[1])
            
            vocal_energy = np.sum(magnitude[vocal_mask, :])
            total_spectral_energy = np.sum(magnitude)
            quality_metrics['vocal_dominance'] = vocal_energy / (total_spectral_energy + 1e-10)
...
def passes_advanced_quality_check(self, track: MusicTrack) -> Tuple[bool, Dict[str, str]]:
    """
    ...
    if self.config.filtering['enable_duration_filter']:
        ...
        if duration < self.config.quality_thresholds['duration_min']:
            reasons['too_short'] = f"Duration {duration:.1f}s < {self.config.quality_thresholds['duration_min']}s"
        elif duration > self.config.quality_thresholds['duration_max']:
            reasons['too_long'] = f"Duration {duration:.1f}s > {self.config.quality_thresholds['duration_max']}s"
    
    # SNR check - only for extremely poor quality audio
    snr = quality_metrics.get('estimated_snr_db', float('inf'))  # Default to pass if no SNR
    if np.isfinite(snr) and snr < self.config.quality_thresholds['snr_min']:
        reasons['low_snr'] = f"SNR {snr:.1f}dB < {self.config.quality_thresholds['snr_min']}dB"
    
    # Dynamic range check - only for extremely compressed audio
    dynamic_range = quality_metrics.get('dynamic_range_db', float('inf'))  # Default to pass
    if np.isfinite(dynamic_range) and dynamic_range < self.config.quality_thresholds['dynamic_range_min']:
        reasons['low_dynamic_range'] = f"Dynamic range {dynamic_range:.1f}dB < {self.config.quality_thresholds['dynamic_range_min']}dB"
    ...
    if spectral_centroid > self.config.quality_thresholds['spectral_centroid_max']:
        reasons['high_spectral_centroid'] = f"Spectral centroid {spectral_centroid:.0f}Hz > {self.config.quality_thresholds['spectral_centroid_max']}Hz"
    ...
    if zcr > self.config.quality_thresholds['zero_crossing_rate_max']:
        reasons['high_zero_crossing_rate'] = f"ZCR {zcr:.3f} > {self.config.quality_thresholds['zero_crossing_rate_max']}"
    ...
    if vocal_dominance < self.config.quality_thresholds['vocal_dominance_min']:
        ...
        self.logger.debug(f"Low vocal dominance {vocal_dominance:.3f} for track {track.track_id}")
        # Don't add to reasons to allow instrumental tracks
    
    # Store quality metrics in track metadata
    track.metadata.update({
        'quality_metrics': quality_metrics,
        'quality_check_passed': len(reasons) == 0
    })
...
def assess_lyrics_quality(self, track: MusicTrack) -> Dict[str, Any]:
    ...
    try:
        ...
        metrics['line_count'] = len([line for line in lyrics.split('\n') if line.strip()])
        ...
        if HAS_LANGDETECT:
            try:
                ...
                if self.config.filtering['enable_language_filter']:
                    metrics['language_allowed'] = detected_lang in self.config.filtering['allowed_languages']
                else:
        ...
        incomplete_indicators = [
            '[instrumental]', '[music]', '[repeat]', '[chorus]', '[verse]',
            '...', 'na na na', 'la la la', 'oh oh oh', 'yeah yeah yeah'
        ]
        
        lyrics_lower = lyrics.lower()
        incomplete_count = sum(1 for indicator in incomplete_indicators if indicator in lyrics_lower)
...
def passes_lyrics_quality_check(self, track: MusicTrack) -> Tuple[bool, Dict[str, str]]:
    ...
    if completeness < self.config.quality_thresholds['lyrics_completeness_min']:
        ...
        self.logger.debug(f"Low lyrics completeness {completeness:.2f} for track {track.track_id}")
    ...
    if self.config.filtering['enable_language_filter']:
        if not lyrics_metrics.get('language_allowed', True):
            ...
            reasons['language_not_allowed'] = f"Language '{detected_lang}' not in allowed list"
    ...
    for i, track in enumerate(tracks):
        if track.audio_data is not None:
            try:
                ...
                mfccs = librosa.feature.mfcc(y=track.audio_data, sr=track.sample_rate, n_mfcc=13)
                spectral_centroid = librosa.feature.spectral_centroid(y=track.audio_data, sr=track.sample_rate)
...
def detect_outliers(self, tracks: List[MusicTrack]) -> List[int]:
    ...
    for i, track in enumerate(tracks):
        if track.audio_data is not None:
            try:
                ...
                self.logger.warning(f"Error extracting features for outlier detection: {e}")
                continue
    ...
    try:
        ...
        outlier_indices = [valid_indices[i] for i, label in enumerate(outlier_labels) if label == -1]
...
def preprocess_track_batch(self, tracks: List[MusicTrack]) -> List[MusicTrack]:
    ...
    for track in tracks:
        ...
        if not audio_passes:
            ...
            self.logger.debug(f"Track {track.track_id} failed audio quality: {audio_reasons}")
            continue
        ...
        if not lyrics_passes:
            ...
            self.logger.debug(f"Track {track.track_id} failed lyrics quality: {lyrics_reasons}")
            continue
        
        processed_tracks.append(track)
    
    # Second pass: duplicate detection
    if self.config.filtering['enable_duplicate_detection'] and len(processed_tracks) > 1:
        ...
        for _, j, similarity in duplicates:
            ...
            self.logger.debug(f"Removing duplicate track at index {j} (similarity: {similarity:.3f})")
        
        processed_tracks = [track for i, track in enumerate(processed_tracks) if i not in duplicate_indices]
    ...
    if self.config.filtering['enable_outlier_detection'] and len(processed_tracks) > 10:
        ...
        processed_tracks = [track for i, track in enumerate(processed_tracks) if i not in outlier_indices]
    ...
    self.logger.info(f"  Lyrics quality failed: {filter_stats['lyrics_quality_failed']}")
