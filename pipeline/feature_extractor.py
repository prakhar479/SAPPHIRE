...
class FeatureExtractor:
    ...
    def extract_features(self, track: MusicTrack) -> FeatureContainer:
        ...
        if track.audio_data is not None and len(track.audio_data) > 0:
            try:
                ...
                if self.config.features.extract_harmony:
                    harmony_features = self._extract_harmony_features(
                        track.audio_data, track.sample_rate
                    )
                ...
                self.logger.error(
                    f"Error extracting audio features for {track.track_id}: {e}"
                )
        ...
        if track.lyrics_text and self.config.features.extract_lyrics:
            try:
                ...
                self.logger.error(
                    f"Error extracting lyrics features for {track.track_id}: {e}"
                )
    ...
    def _extract_spectral_features(self, y: np.ndarray, sr: int) -> Dict[str, Any]:
        ...
        features.update({"spectral_rolloff_mean": float(np.mean(rolloff))})
    ...
    def _extract_advanced_spectral_features(
        ...
        try:
            ...
            for i in range(tonnetz.shape[0]):
                features.update({f"tonnetz_{i}_mean": float(np.mean(tonnetz[i]))})
        ...
        try:
            ...
            if len(beats) > 2:
                ...
                features["rhythm_regularity"] = float(
                    1.0 - (np.std(beat_intervals) / np.mean(beat_intervals))
                )
        ...
        try:
            ...
            if np.mean(np.abs(y)) > 0:
                features["dynamic_range_db"] = float(
                    20 * np.log10(np.max(np.abs(y)) / np.mean(np.abs(y)))
                )
            else:
        ...
        try:
            ...
            if self.sentence_model:
                try:
                    ...
                    features["semantic_embedding_norm"] = float(
    ...
    def _estimate_key(
        self, chroma_mean: np.ndarray, key_profiles: Dict[str, np.ndarray]
    ) -> Dict[str, Any]:
        ...
        if correlations:
            ...
            return {"estimated_key": best_key[0], "key_confidence": float(best_key[1])}
        else:
            return {"estimated_key": "unknown", "key_confidence": 0.0}
    ...
    def extract_batch(
        ...
        if n_workers == 1:
            ...
            return [
                self.extract_features(track)
                for track in tqdm(tracks, desc="Extracting features")
            ]
        else:
            # Parallel processing
            with ProcessPoolExecutor(max_workers=n_workers) as executor:
                futures = [
                    executor.submit(self.extract_features, track) for track in tracks
                ]
                results = []
