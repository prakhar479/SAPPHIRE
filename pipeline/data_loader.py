...
class DataLoader:
    ...
    def load_mirex_mood_dataset(self) -> List[MusicTrack]:
        ...
        for i, audio_file in enumerate(sorted(audio_files)):
            ...
            mood_category = categories[file_index] if file_index < len(categories) else None
    ...
    def load_lyrics(self, track: MusicTrack) -> MusicTrack:
        ...
        try:
            ...
            track.metadata.update({
                'lyrics_loaded': True,
                'lyrics_length': len(track.lyrics_text)
            })
    ...
    def load_all_datasets(self) -> List[MusicTrack]:
        ...
        for dataset_name, loader_func in dataset_loaders:
            try:
                ...
                self.logger.info(f"Successfully loaded {len(tracks)} tracks from {dataset_name}")
            except Exception as e:
        ...
        for dataset_name, config in dataset_patterns.items():
            ...
            if dataset_path.exists():
                ...
                if 'audio_subdir' in config:
                    ...
                    if audio_dir.exists():
                        info['audio_files'] = len(list(audio_dir.glob('*.mp3')) + list(audio_dir.glob('*.wav')))
                ...
                if 'subdirs' in config:
                    ...
                    for subdir in config['subdirs']:
                        ...
                        if subdir_path.exists():
                            total_audio += len(list(subdir_path.glob('**/*.mp3')) + list(subdir_path.glob('**/*.wav')))
    ...
    def get_dataset_statistics(self, tracks: List[MusicTrack]) -> Dict:
        ...
        stats = {
            ...
            'tracks_with_audio': sum(1 for t in tracks if t.audio_path and os.path.exists(t.audio_path)),
            'tracks_with_lyrics': sum(1 for t in tracks if t.lyrics_path and os.path.exists(t.lyrics_path)),
            'datasets': {},
            'mood_clusters': {},
            'mood_categories': {},
            'languages': {}
        ...
        for track in tracks:
            ...
            if track.mood_cluster:
                stats['mood_clusters'][track.mood_cluster] = stats['mood_clusters'].get(track.mood_cluster, 0) + 1
            if track.mood_category:
                stats['mood_categories'][track.mood_category] = stats['mood_categories'].get(track.mood_category, 0) + 1
