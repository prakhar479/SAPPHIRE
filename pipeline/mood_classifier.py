...
class MoodClassifier:
    ...
    def prepare_data(self, features_df: pd.DataFrame, target_column: str = 'mood_cluster') -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def select_features(self, X: np.ndarray, y: np.ndarray, method: str = 'mutual_info', k: int = None) -> np.ndarray:
        ...
        if method == 'mutual_info':
            ...
            estimator = RandomForestClassifier(n_estimators=100, random_state=self.config.model.random_state)
        ...
        if hasattr(self.feature_selector, 'get_support'):
            ...
            self.logger.info(f"Selected features: {selected_features[:10]}...")  # Show first 10
    ...
    def train_models(self, X: np.ndarray, y: np.ndarray) -> Dict[str, Dict]:
        ...
        for model_name in self.config.model.models:
            ...
            try:
                ...
                cv_scores = cross_val_score(best_model, X_train_scaled, y_train, cv=self.config.model.cv_folds)
                ...
                self.logger.info(f"{model_name} - Test Accuracy: {accuracy:.4f}, CV: {cv_scores.mean():.4f} ± {cv_scores.std():.4f}")
        ...
        if results:
            ...
            self.logger.info(f"Best model: {best_model_name} (Accuracy: {results[best_model_name]['test_accuracy']:.4f})")
    ...
    def evaluate_models(self, results: Dict[str, Dict], output_dir: str):
        ...
        plt.errorbar(x + width/2, cv_means, yerr=cv_stds, fmt='o', label='CV Mean ± Std', capsize=5)
    ...
    def predict(self, features: Union[np.ndarray, pd.DataFrame]) -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def cross_modal_analysis(self, features_df: pd.DataFrame) -> Dict[str, Any]:
        ...
        acoustic_features = [col for col in features_df.columns if any(
            keyword in col.lower() for keyword in ['mfcc', 'spectral', 'chroma', 'tempo', 'rhythm']
        )]
        
        lyrical_features = [col for col in features_df.columns if any(
            keyword in col.lower() for keyword in ['sentiment', 'embedding', 'readability', 'lyrics']
        )]
        ...
        if acoustic_features and lyrical_features:
            ...
            self.logger.info(f"Cross-modal analysis complete. Mean Jaccard similarity: {np.mean(jaccard_scores):.4f}")
