...
class Visualizer:
    ...
    def create_feature_distribution_plots(
        ...
        for i in range(0, len(numeric_cols), batch_size):
            ...
            plt.tight_layout()
            plt.savefig(
                output_path / f"distributions_batch_{i//batch_size + 1}.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
    ...
    def create_mood_analysis_plots(self, features_df: pd.DataFrame, output_dir: str):
        ...
        if "mood_cluster" not in features_df.columns:
            self.logger.warning(
                "No mood_cluster column found, skipping mood analysis plots"
            )
            return
        ...
        important_features = [
            ...
            if any(
                ...
                for keyword in ["mfcc", "spectral", "tempo", "sentiment", "energy"]
            )
        ][
            :12
        ...
        if important_features:
            ...
            plt.suptitle(
                "Feature Comparison Across Mood Clusters",
                fontsize=16,
                fontweight="bold",
            )
            plt.tight_layout()
            plt.savefig(
                output_path / "mood_feature_comparison.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
    ...
    def create_correlation_heatmap(
        self, features_df: pd.DataFrame, output_dir: str, max_features: int = 50
    ):
        """
        ...
        corr_data = (
            features_df[selected_cols].replace([np.inf, -np.inf], np.nan).fillna(0)
        )
    ...
    def create_pca_visualization(self, features_df: pd.DataFrame, output_dir: str):
        ...
        plt.tight_layout()
        plt.savefig(
            output_path / "pca_explained_variance.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
        ...
        if "mood_cluster" in features_df.columns:
            ...
            plt.tight_layout()
            plt.savefig(
                output_path / "pca_mood_clusters.png", dpi=300, bbox_inches="tight"
            )
            plt.close()
    ...
    def create_tsne_visualization(self, features_df: pd.DataFrame, output_dir: str):
        ...
        tsne = TSNE(
            n_components=2, random_state=42, perplexity=min(30, len(X_tsne_input) - 1)
        )
        ...
        if "mood_cluster" in features_subset.columns:
            ...
            plt.tight_layout()
            plt.savefig(
                output_path / "tsne_mood_clusters.png", dpi=300, bbox_inches="tight"
            )
            plt.close()

        self.logger.info(f"t-SNE visualization saved to {output_path}")

    def create_cross_modal_plots(
        self, cross_modal_results: Dict[str, Any], output_dir: str
    ):
        """
        ...
        if (
            "correlations" in cross_modal_results
            and cross_modal_results["correlations"]
        ):
            ...
            plt.tight_layout()
            plt.savefig(
                output_path / "cross_modal_correlations.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
        ...
        if (
            "jaccard_similarity" in cross_modal_results
            and cross_modal_results["jaccard_similarity"]
        ):
            ...
            plt.tight_layout()
            plt.savefig(
                output_path / "jaccard_similarity_distribution.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
    ...
    def create_model_performance_plots(
        self, model_results: Dict[str, Dict], output_dir: str
    ):
        """
        ...
        bars1 = ax1.bar(
            models, test_accuracies, color="lightblue", alpha=0.8, edgecolor="black"
        )
    ...
    def create_feature_importance_plot(
        self, importance_df: pd.DataFrame, output_dir: str, top_n: int = 20
    ):
        """
    ...
    def create_comprehensive_report(
        ...
        if "cross_modal_analysis" in analysis_results:
            self.create_cross_modal_plots(
                analysis_results["cross_modal_analysis"], str(output_path)
            )
        ...
        summary_stats = {
            ...
            "mood_distribution": (
                features_df["mood_cluster"].value_counts().to_dict()
                if "mood_cluster" in features_df.columns
                else {}
            ),
            "dataset_distribution": (
                features_df["dataset"].value_counts().to_dict()
