...
def cmd_extract_features(args):
    ...
    print(
        f"🔢 Total features per track: {len(features_df.columns) - 4}"
    )  # -4 for metadata
...
def cmd_train_models(args):
    ...
    if getattr(args, "normalize", False):
        ...
        if getattr(args, "normalize_out", None):
            ...
            normalized_path = features_path.with_name(
                features_path.stem + "_clean.parquet"
            )
        ...
        try:
            ...
            subprocess.run(
                [
                    sys.executable,
                    "scripts/normalize_feature_columns.py",
                    str(features_path),
                    str(normalized_path),
                ],
                check=True,
    ...
    if getattr(args, "top_n_important", None):
        ...
        importance_csv_path = (
            args.important_features_csv or "output/fi_test/feature_importances_full.csv"
        )
        labeled_tracks, used_importance_df = classifier.subset_features_by_importance(
            labeled_tracks, importance_csv_path, args.top_n_important
        )
        ...
        if not used_importance_df.empty:
            ...
            print(
                f"✨ Using {len(used_importance_df)} important features (top {args.top_n_important}) from {importance_csv_path}"
            )
            print(
                f"   First features: {used_importance_df['feature'].head(10).tolist()}"
            )
        else:
            print(
                "⚠️  Importance-based subsetting requested but no usable features were found; proceeding with original features"
            )
...
def cmd_pipeline(args):
    ...
    if getattr(args, "top_n_important", None):
        ...
        config_obj.model.importance_csv_path = (
            args.important_features_csv
            or config_obj.model.importance_csv_path
            or "output/fi_test/feature_importances_full.csv"
        )
...
def cmd_status(args):
    ...
    if args.action == "show":
        ...
        processor = EnhancedProcessingPipeline(
        ...
        print(
            f"  Usage: {status['memory_usage_gb']/status['memory_limit_gb']*100:.1f}%"
        )
        ...
        processor = EnhancedProcessingPipeline(
            config_obj, checkpoint_dir=args.checkpoint_dir or "checkpoints"
        )
...
def cmd_config(args):
    ...
    if args.action == "show":
        ...
        print(
            f"  Duplicate Detection: {config.filtering['enable_duplicate_detection']}"
        )
        ...
        print(
            f"  Incremental Processing: {config.optimization['incremental_processing']}"
        )
        print(
            f"  Resume from Checkpoint: {config.optimization['resume_from_checkpoint']}"
        )
...
Examples:
  ...
  python cli.py predict --audio song.mp3 --model models/best_model.joblib
    ...
    discover_parser = subparsers.add_parser(
        "discover", help="Discover available datasets"
    )
    discover_parser.set_defaults(func=cmd_discover)

    # Extract features command
    extract_parser = subparsers.add_parser(
        "extract-features", help="Extract features from datasets"
    )
    extract_parser.add_argument(
        "--datasets",
        type=str,
        help="Comma-separated list of datasets (mirex,csd,jam-alt,vietnamese)",
    )
    extract_parser.add_argument(
    ...
    train_parser.add_argument(
        ...
        type=str,
        help="Path to feature importance CSV for RF-based selection (default: output/fi_test/feature_importances_full.csv)",
    )
    train_parser.add_argument(
        "--top-n-important",
        type=int,
        help="Use top-N features from importance CSV before model training",
    )
    train_parser.add_argument(
        "--normalize",
        action="store_true",
    ...
    analyze_parser = subparsers.add_parser(
        "analyze", help="Perform comprehensive analysis"
    )
    ...
    predict_parser = subparsers.add_parser(
        "predict", help="Predict mood for a single track"
    )
    ...
    predict_parser.add_argument(
        "--model", type=str, required=True, help="Trained model file path"
    )
    ...
    pipeline_parser.add_argument(
        "--top-n-important",
        type=int,
        help="Use top-N important features during classifier training",
    )
    pipeline_parser.add_argument(
    ...
    config_subparsers = config_parser.add_subparsers(
        dest="action", help="Configuration actions"
    )
    ...
    config_create = config_subparsers.add_parser(
        "create", help="Create configuration file"
    )
    config_create.add_argument(
        "--output", type=str, required=True, help="Output configuration file path"
    )
    ...
    config_validate.add_argument(
        "--file", type=str, required=True, help="Configuration file to validate"
    )
    ...
    status_parser = subparsers.add_parser(
        "status", help="Manage processing status and checkpoints"
    )
    status_subparsers = status_parser.add_subparsers(
        dest="action", help="Status actions"
    )
    ...
    status_cleanup = status_subparsers.add_parser(
        "cleanup", help="Clean up checkpoints"
    )
    status_cleanup.add_argument(
        "--checkpoint-dir", type=str, help="Checkpoint directory"
    )

    status_resume = status_subparsers.add_parser(
        "resume", help="Resume processing from checkpoints"
    )
    status_resume.add_argument(
        "--checkpoint-dir", type=str, help="Checkpoint directory"
    )
    status_resume.add_argument(
        "--datasets", type=str, help="Comma-separated list of datasets"
