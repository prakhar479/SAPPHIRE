...
def analyze_audio_file(audio_path, label, embedder_data):
    ...
    print(f"    Vector stats BEFORE scaling: min={x.min():.4f}, max={x.max():.4f}, mean={x.mean():.4f}, std={x.std():.4f}")
    ...
    if scaler is not None:
        ...
        x = np.clip(x, -100, 100)
        print(f"    Vector stats AFTER scaling+clipping: min={x.min():.4f}, max={x.max():.4f}, mean={x.mean():.4f}, std={x.std():.4f}")
        print(f"    First 10 values: {x[0][:10]}")
...
def main():
    ...
    if not embedder:
        ...
        return
    print(f"Embedder loaded. Input dim: {embedder['input_dim']}, Embedding dim: {embedder['embedding_dim']}")
    ...
    if index and len(embeddings) >= 2:
        ...
        neighbors1, distances1 = index.get_nns_by_vector(embeddings[0].tolist(), 5, include_distances=True)
        print(f"  Top 5 neighbors: {neighbors1}")
        print(f"  Distances: {distances1}")
        
        print(f"\nSearching with second embedding...")
        neighbors2, distances2 = index.get_nns_by_vector(embeddings[1].tolist(), 5, include_distances=True)
