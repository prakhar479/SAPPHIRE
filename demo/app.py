...
def robust_load_audio(path, sr=22050):
    """Load audio using ffmpeg conversion to WAV if needed."""
...
def load_index(expected_dim=None):
    ...
    if dim is None:
        ...
        logger.warning(
            "load_index: no index metadata available and no expected_dim provided"
        )
        return None, None
    ...
    for d in model_dirs:
        ...
        if os.path.exists(ckpt_path):
            ...
            try:
                ...
                info["inferred_num_classes"] = (
                    int(ckpt.get("num_classes")) if ckpt.get("num_classes") else None
                )
                state_dict = (
                ...
                if state_dict is not None:
                    ...
                    info["sample_feature_keys"] = {
                        k: (
                            list(state_dict[k].shape)
                            if hasattr(state_dict[k], "shape")
                            else str(type(state_dict[k]))
                        )
                        for k in feat_keys[:20]
                    }
                    ...
                    for k in sorted(
                        ...
                        key=lambda x: int(
                            re.match(r"feature_extractor\.(\d+)\.weight", x).group(1)
                        ),
                    ):
                        ...
                        if (
                            hasattr(tensor, "ndim")
                            and int(getattr(tensor, "ndim", 0)) == 2
                        ):
                            idx = int(
                                re.match(r"feature_extractor\.(\d+)\.weight", k).group(
                                    1
                                )
                            )
                    ...
                    hidden_dims = [
                        out for (_, out) in sorted(linear_entries, key=lambda x: x[0])
                    ]
                    if hidden_dims:
...
def api_upload():
    ...
    if "file" not in request.files:
        return (
            jsonify(
                {"ok": False, "error": 'no file uploaded (field name should be "file")'}
            ),
            400,
        )
    ...
    try:
        ...
        try:
            ...
            return (
                jsonify({"ok": False, "error": f"feature extraction failed: {e}"}),
                500,
            )
        ...
        if not _CACHED_EMBEDDER or _CACHED_EMBEDDER.get("model") is None:
            return (
                jsonify(
                    {
                        "ok": False,
                        "error": "embedder checkpoint not available or torch not installed",
                    }
                ),
                500,
            )
        ...
        if scaler is not None:
            try:
                ...
                # Some placeholder features have tiny scale values causing explosions
                x = np.clip(x, -100, 100)
            except Exception as e:
                pass
        ...
        try:
            ...
            if (
                ...
                and len(getattr(model, "feature_extractor")) > 0
            ):
        ...
        if x.shape[1] < expected:
            ...
            logger.warning(
                "input feature length (%d) > model expected (%d); trimming extra features",
                x.shape[1],
                expected,
            )
            x = x[:, :expected]
        ...
        if embeddings is None:
            return (
                jsonify(
                    {"ok": False, "error": "embeddings not found (build index first)"}
                ),
                400,
            )
        ...
        if index is None or index_dim is None:
            return (
                jsonify(
                    {
                        "ok": False,
                        "error": "index not built yet (or index metadata missing)",
                    }
                ),
                400,
            )
        ...
        if len(emb) != int(index_dim):
            return (
                jsonify(
                    {
                        "ok": False,
                        "error": f"embedding dimension mismatch: index expects {index_dim}, got {len(emb)}. Rebuild the index with the same embedder.",
                    }
                ),
                400,
            )
        neighbors, distances = index.get_nns_by_vector(
            emb.tolist(), k, include_distances=True
        )
        meta = load_metadata()
        results = []
...
def api_add_to_library():
    ...
    for file in files:
        ...
        try:
            ...
            try:
                with torch.no_grad():
                    xth = torch.from_numpy(x)
                    out = model(xth)
                    # Handle tuple return (model might return (embeddings, aux_output))
...
def find_feature_columns():
    ...
    candidates = [
        os.path.join(
            BASE_DIR, "features", "mirex", "features_flat_clean_clean.parquet"
        ),
        ...
        if os.path.exists(p):
            try:
                ...
                cols = [
                    ...
                    for c in df.columns
                    if c not in exclude
                    and not str(c).startswith("extra.")
                    and "track_id" not in str(c)
                ]
    ...
    def _visit(prefix, obj):
                ...
                for i, v in enumerate(obj):
                    ...
                    if '.' in prefix:
                        ...
                        key = f"{parts[0]}.mfcc_{i}_" + (
                            ...
                            else "std" if "std" in base_name else "val"
                        )
                    else:
                        key = f"mfcc_{i}_" + (
                            "mean"
                            if "mean" in prefix
                            else "std" if "std" in prefix else "val"
                        )
                    try:
        ...
        if isinstance(obj, dict):
            for k, v in obj.items():
                # Construct new prefix with dot notation
                ...
                if k == "mfcc_mean":
                    ...
                    _visit(new_prefix, v)
                elif k == "spectral_contrast_mean":
                    _visit(new_prefix, v)
                elif k == "spectral_contrast_std":
                    _visit(new_prefix, v)
                elif k == "tonnetz_mean":
                    _visit(new_prefix, v)
                elif k == "tonnetz_std":
...
def load_embedder_checkpoint(prefer="embedder_mirex"):
    ...
    try:
        ...
        state_dict = (
            ckpt.get("model_state_dict") or ckpt.get("model") or ckpt.get("model_state")
        )
        inferred_hidden_dims = None
        inferred_num_classes = (
            int(ckpt.get("num_classes", 0)) if ckpt.get("num_classes") else None
        )
        if state_dict is not None:
            try:
                import re
                ...
                feat_keys = [
                    k
                    for k in state_dict.keys()
                    if k.startswith("feature_extractor.") and k.endswith(".weight")
                ]
                linear_entries = []
                for k in sorted(
                    feat_keys,
                    ...
                    if hasattr(tensor, "ndim") and int(getattr(tensor, "ndim", 0)) == 2:
                        idx = int(
                            re.match(r"feature_extractor\.(\d+)\.weight", k).group(1)
                        )
                        outf = int(tensor.shape[0])
                        linear_entries.append((idx, outf))
        ...
        class SongEmbedderSimple(torch.nn.Module):
            def __init__(
                ...
                self.feature_extractor = (
                    torch.nn.Sequential(*layers) if layers else torch.nn.Identity()
                )
                self.embedding_layer = torch.nn.Linear(prev, embedding_dim)
                self.classifier = torch.nn.Linear(
                    embedding_dim, int(ckpt.get("num_classes", num_classes))
                )
        ...
        model = SongEmbedderSimple(
            input_dim, embedding_dim, num_classes=int(ckpt.get("num_classes", 2))
        )
        ...
        if state_dict is None:
            logger.error(
                "no model_state found in checkpoint keys: %s", list(ckpt.keys())
            )
            return None
    ...
    if embeddings is None:
        return (
            jsonify(
                {
                    "ok": False,
                    "error": "No embeddings found (looked in models/embedder_mirex/embeddings)",
                }
            ),
            400,
        )
        ...
        if emb.ndim == 1 and int(emb.size) != int(index_dim):
            return (
                jsonify(
                    {
                        "ok": False,
                        "error": f"embedding dimension mismatch: index expects {index_dim}, got {emb.size}. Rebuild the index with the same embedder.",
                    }
                ),
                400,
            )
