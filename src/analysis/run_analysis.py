...
def load_and_prepare_data(manifest_path: Path) -> pd.DataFrame:
    ...
    df = df.drop(
        columns=[col for col in df.columns if "cleaned_text" in col or "path" in col],
        errors="ignore",
    )
...
def perform_univariate_analysis(
    df: pd.DataFrame, output_dir: Path, feature_cols: List[str]
):
...
def perform_correlation_analysis(
    df: pd.DataFrame, acoustic_cols: List[str], lyrics_cols: List[str], output_dir: Path
):
    ...
    cross_corr = pd.concat([numeric_acoustic_df, numeric_lyrics_df], axis=1).corr(
        method="spearman"
    )
    # Select the rectangle of interest from the full correlation matrix
    cross_corr_subset = cross_corr.loc[
        numeric_lyrics_df.columns, numeric_acoustic_df.columns
    ]
    plt.figure(figsize=(20, 8))
    sns.heatmap(
        cross_corr_subset, annot=True, cmap="PRGn", center=0, fmt=".2f", linewidths=0.5
    )
...
def perform_pca_analysis(df: pd.DataFrame, acoustic_cols: List[str], output_dir: Path):
    ...
    REPORT_LINES.append(
        f"The first two principal components explain **{explained_variance:.2%}** of the variance in the acoustic features.\n"
    )
    loadings = pd.DataFrame(
        pca.components_[:3, :].T, columns=["PC1", "PC2", "PC3"], index=acoustic_cols
    )
    REPORT_LINES.append("### Top Feature Loadings for PCs:\n")
    REPORT_LINES.append(
        f"**PC1 Interpretation:** Features with largest positive loadings: `{loadings['PC1'].idxmax()}`. Features with largest negative loadings: `{loadings['PC1'].idxmin()}`.\n"
    )
    REPORT_LINES.append(
        f"**PC2 Interpretation:** Features with largest positive loadings: `{loadings['PC2'].idxmax()}`. Features with largest negative loadings: `{loadings['PC2'].idxmin()}`.\n"
    )
...
def perform_cross_modal_similarity_analysis(
    main_df: pd.DataFrame, manifest_path: Path, acoustic_cols: List[str]
):
    ...
    df_text = pd.read_parquet(
        manifest_path, columns=["track_id", "lyrics_features_cleaned_text"]
    )
    ...
    X_lyrics_embeddings = model.encode(
        lyrics_list, show_progress_bar=True, batch_size=128
    )
    ...
    acoustic_neighbors_idx = nn_acoustic.kneighbors(
    ...
    lyric_neighbors_idx = nn_lyrics.kneighbors(
        X_lyrics_embeddings, return_distance=False
    )
    ...
    tasks = [
        (i, acoustic_neighbors_idx, lyric_neighbors_idx) for i in range(len(main_df))
    ]

    with ProcessPoolExecutor() as executor:
        futures = [executor.submit(_get_neighbor_overlap, task) for task in tasks]
        for future in tqdm(
            as_completed(futures), total=len(tasks), desc="Calculating Jaccard scores"
        ):
            jaccard_scores.append(future.result())

    mean_jaccard = np.mean(jaccard_scores)
    REPORT_LINES.append(
        f"The average Jaccard similarity between the sets of top {K} acoustic and lyrical neighbors is: **{mean_jaccard:.4f}**\n"
    )
    REPORT_LINES.append(
...
def main(args):
    ...
    if not args.skip_similarity:
        perform_cross_modal_similarity_analysis(
            df, Path(args.manifest_path), acoustic_cols
        )
    else:
