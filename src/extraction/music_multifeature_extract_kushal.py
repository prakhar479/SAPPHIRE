...
def load_audio_mono(path: str, sr: int = 22050, offset: float = 0.0, duration: Optional[float] = None):
    ...
    try:
        ...
        logging.warning(f"librosa failed to load {path}: {e}. Will attempt ffmpeg transcode fallback.")
    ...
    def _try_hf_snapshot_copy(basename: str, dst_path: str) -> bool:
        ...
        if not HAS_HF_HUB:
            logging.debug("huggingface_hub not available; skipping HF snapshot fallback")
            return False

        repo = os.environ.get('HF_DATASET_ID', 'jamendolyrics/jam-alt')
        logging.info(f"Attempting HF snapshot download from {repo} to recover {basename}")
        tmp_dir = None
        try:
    ...
    try:
        ...
        if _try_ffmpeg(path, tmp_wav):
            y, sr_loaded = librosa.load(tmp_wav, sr=sr, mono=True, offset=offset, duration=duration)
            if y.size > 0:
                max_val = np.max(np.abs(y))
                if max_val > 0:
                    y = y / max_val
        ...
        with tempfile.NamedTemporaryFile(suffix=os.path.splitext(path)[1], delete=False) as tmp_src:
...
def process_single_file(audio_path: str, lyrics_path: Optional[str], out_dir: str, config: Config) -> Dict[str, Any]:
    ...
    if getattr(config, 'force_transcode', False):
        try:
            trans_dir = config.transcode_dir or os.path.join(os.path.dirname(out_dir), 'transcoded')
...
def batch_process(pairs: List[Tuple[str, Optional[str]]], out_dir: str, config: Config):
    ...
    for audio_path, lyrics_path in pairs:
        ...
        if os.path.exists(realp):
            ...
            logging.warning(f"Audio file not found (target missing): {audio_path} -> {realp}")
...
Examples:
  ...
  python %(prog)s --audio song.mp3 --quick --no-plots
    ...
    audio_group.add_argument('--force-transcode', action='store_true', help='Transcode inputs to WAV before analysis')
    audio_group.add_argument('--transcode-dir', type=str, help='Directory to place transcoded WAV files')
