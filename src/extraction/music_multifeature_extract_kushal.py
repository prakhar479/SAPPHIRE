...
def setup_logging(verbose: bool = False):
    ...
    logging.basicConfig(
        ...
        handlers=[logging.FileHandler("music_extraction.log"), logging.StreamHandler()],
    )
...
def sha1_of_file(path: str, block_size: int = 65536) -> str:
    ...
    # Resolve symlink targets so we hash the real file contents when audio files are symlinks
...
# Enhanced audio loading with error handling
def load_audio_mono(
    path: str, sr: int = 22050, offset: float = 0.0, duration: Optional[float] = None
):
    """Load audio file with enhanced error handling and normalization."""
    # Resolve symlinks so decoders operate on the real file
    path = os.path.realpath(path)

    # First attempt: librosa.load
    try:
        y, sr_loaded = librosa.load(
            path, sr=sr, mono=True, offset=offset, duration=duration
        )
        # Normalize
        if y.size > 0:
        ...
        logging.warning(
            f"librosa failed to load {path}: {e}. Will attempt ffmpeg transcode fallback."
        )

    # Fallback sequence: try ffmpeg -> mpg123 -> sox -> huggingface snapshot copy -> give up
    def _try_ffmpeg(src: str, dst_wav: str) -> bool:
        cmd = [
            "ffmpeg",
            "-y",
            "-hide_banner",
        ...
        try:
            ...
            if proc.returncode == 0:
                return True
            logging.debug(f"ffmpeg stderr: {proc.stderr}")
            return False
        except FileNotFoundError:
            logging.debug("ffmpeg not found on PATH")
            return False

    def _try_mpg123(src: str, dst_wav: str) -> bool:
        # mpg123 -w out.wav in.mp3
        cmd = ["mpg123", "-q", "-w", dst_wav, src]
        try:
            proc = subprocess.run(
                cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
            )
            if proc.returncode == 0:
                return True
            logging.debug(f"mpg123 stderr: {proc.stderr}")
            return False
        except FileNotFoundError:
            logging.debug("mpg123 not found on PATH")
            return False

    def _try_sox(src: str, dst_wav: str) -> bool:
        # sox in.mp3 -r SR -c 1 out.wav
        cmd = ["sox", src, "-r", str(sr), "-c", "1", dst_wav]
        try:
            proc = subprocess.run(
                cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True
            )
            if proc.returncode == 0:
                return True
            logging.debug(f"sox stderr: {proc.stderr}")
            return False
        except FileNotFoundError:
            logging.debug("sox not found on PATH")
            return False

    def _try_hf_snapshot_copy(basename: str, dst_path: str) -> bool:
        # Try to download dataset snapshot from HF and copy the matching file
        if not HAS_HF_HUB:
            logging.debug(
                "huggingface_hub not available; skipping HF snapshot fallback"
            )
            return False

        repo = os.environ.get("HF_DATASET_ID", "jamendolyrics/jam-alt")
        logging.info(
            f"Attempting HF snapshot download from {repo} to recover {basename}"
        )
        tmp_dir = None
        try:
            tmp_dir = tempfile.mkdtemp()
            snapshot_dir = snapshot_download(repo_id=repo, cache_dir=tmp_dir)
                ...
                for f in files:
                    if os.path.splitext(f)[0] == basename:
                        ...
                        try:
                            ...
                            logging.info(f"Copied {src} from HF snapshot to {dst_path}")
                            return True
                        except Exception:
                            continue
        except Exception as e:
            logging.debug(f"HF snapshot download/copy error: {e}")
        finally:
            if tmp_dir:
                try:
    ...
    try:
        ...
        # 1) ffmpeg
        if _try_ffmpeg(path, tmp_wav):
            y, sr_loaded = librosa.load(
                tmp_wav, sr=sr, mono=True, offset=offset, duration=duration
            )
            if y.size > 0:
                max_val = np.max(np.abs(y))
                if max_val > 0:
        ...
        # 4) Hugging Face snapshot copy + ffmpeg
        ...
        if _try_hf_snapshot_copy(basename, tmp_src_path):
            try:
                if _try_ffmpeg(tmp_src_path, tmp_wav):
                    y, sr_loaded = librosa.load(
                        tmp_wav, sr=sr, mono=True, offset=offset, duration=duration
                    )
                    if y.size > 0:
        ...
        logging.error(f"All decode/transcode fallbacks failed for {path}")
        return np.array([]), sr, 0.0
    except Exception as e:
        logging.error(f"Error during fallback decoding for {path}: {e}")
        return np.array([]), sr, 0.0


# Advanced spectral features
def advanced_spectral_features(
    ...
    try:
        ...
        for frame in S.T:
            if np.sum(frame) > 0:
                ...
                skewness = np.sum(((freqs - mean_freq) ** 3) * frame_norm) / (
                    spread**3 + 1e-10
                )
                kurtosis = np.sum(((freqs - mean_freq) ** 4) * frame_norm) / (
                    spread**4 + 1e-10
                )
                ...
                if np.sum(valid_idx) > 1:
                    slope, _ = np.polyfit(
                        freqs[valid_idx], np.log(frame[valid_idx] + 1e-10), 1
                    )
                else:
                    slope = 0
...
def enhanced_rhythm_analysis(
    ...
    try:
        ...
        tempo_confidence = (
            1.0 / (1.0 + np.std(beat_intervals)) if len(beat_intervals) > 0 else 0.0
        )
...
def enhanced_harmonic_analysis(
    y: np.ndarray, sr: int, hop_length: int = 512
) -> Dict[str, Any]:
...
def enhanced_lyrics_analysis(text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        repetition_ratio = (
            1.0 - (unique_words / total_words) if total_words > 0 else 0.0
        )
...
def analyze_audio_comprehensive(
    audio_path: str, lyrics_path: Optional[str], config: Config
) -> Dict[str, Any]:
    ...
    try:
        ...
        # Advanced spectral features
        features["acoustic"].update(
            advanced_spectral_features(y, sr, config.hop_length)
        )
...
def save_structured_outputs(
    base_out_dir: str, basename: str, features: Dict[str, Any], config: Config
) -> str:
    """Save structured outputs with enhanced organization."""
    ensure_dir(base_out_dir)

    file_hash = (
        features.get("meta", {}).get("file_hash")
        or hashlib.sha1(basename.encode()).hexdigest()[:8]
    )
...
def transcode_file_to_wav(src_path: str, out_dir: str, sr: int) -> Optional[str]:
    """Transcode an input audio file to a single-channel WAV at sample rate sr using ffmpeg.
    ...
    try:
        ...
        dst_path = os.path.join(out_dir, f"{stem}.wav")

        # If destination already exists, assume it's from a previous transcode
        ...
        if proc.returncode != 0:
            logging.error(f"ffmpeg transcode failed for {src_path}: {proc.stderr}")
            return None

        return dst_path
    except FileNotFoundError:
        logging.error("ffmpeg not found on PATH; cannot transcode files.")
        return None
    except Exception as e:
        logging.error(f"Unexpected error during transcode of {src_path}: {e}")
        return None


def process_single_file(
    audio_path: str, lyrics_path: Optional[str], out_dir: str, config: Config
) -> Dict[str, Any]:
    """Process a single audio file."""
    start_time = time.time()

    # If force_transcode is requested, attempt to create a WAV copy first
    if getattr(config, "force_transcode", False):
        try:
            trans_dir = config.transcode_dir or os.path.join(
                os.path.dirname(out_dir), "transcoded"
            )
            ensure_dir(trans_dir)
            # Only transcode if not already WAV
            if not audio_path.lower().endswith(".wav"):
                transcoded = transcode_file_to_wav(audio_path, trans_dir, config.sr)
                if transcoded:
                    audio_path = transcoded
        except Exception as e:
            logging.warning(f"Transcode step failed for {audio_path}: {e}")

    # Keep the original path for metadata but operate on the real file target (follow symlinks)
    original_path = audio_path
    real_audio_path = os.path.realpath(audio_path)
    basename = Path(real_audio_path).stem

    try:
        # Check if already processed
        # Hash the real file target (so symlinked files are deduplicated correctly)
        current_hash = sha1_of_file(real_audio_path)
        ...
        # Perform analysis: call analysis on the real file target but preserve original path in metadata
        features = analyze_audio_comprehensive(real_audio_path, lyrics_path, config)
        # store original path as seen
        features.get("meta", {})["original_path"] = original_path
...
def gather_audio_files(
    ...
    # followlinks=True ensures we traverse into any symlinked directories (e.g., audio -> subsets)
...
def batch_process(pairs: List[Tuple[str, Optional[str]]], out_dir: str, config: Config):
    ...
    for audio_path, lyrics_path in pairs:
        realp = os.path.realpath(audio_path)
        if os.path.exists(realp):
            # pass the original path (may be a symlink) so outputs keep the expected names
            valid_pairs.append((audio_path, lyrics_path))
        else:
            logging.warning(
                f"Audio file not found (target missing): {audio_path} -> {realp}"
            )

    if not valid_pairs:
        (
            console.print("[red]No valid audio files found![/red]")
            if HAS_RICH
            else print("No valid audio files found!")
        )
        return

    total_files = len(valid_pairs)
    (
        console.print(f"[green]Processing {total_files} audio files...[/green]")
        if HAS_RICH
        else print(f"Processing {total_files} audio files...")
    )
        ...
        with mp.Pool(processes=config.workers) as pool:
            if HAS_RICH:
                    ...
                    for result in pool.imap_unordered(enhanced_worker, worker_args):
                        ...
                        if result["status"] == "success":
                            progress.console.print(
                                f"[green]✓[/green] {os.path.basename(result['audio'])}"
                            )
                        elif result["status"] == "error":
                            progress.console.print(
                                f"[red]✗[/red] {os.path.basename(result['audio'])}: {result['error']}"
                            )
                        else:
                            progress.console.print(
                                f"[yellow]⚠[/yellow] {os.path.basename(result['audio'])}: skipped"
                            )
                ...
                for i, result in enumerate(
                    pool.imap_unordered(enhanced_worker, worker_args), 1
                ):
                    results[result["status"]] += 1
                    status_symbol = (
                        "✓"
                    ...
                    print(
                        f"[{i}/{total_files}] {status_symbol} {os.path.basename(result['audio'])}"
                    )
    else:
...
def print_analysis_summary(output_folder: str):
    ...
    try:
        ...
        if HAS_RICH:
            ...
            console.print(
                f"[bold]Duration:[/bold] {meta.get('duration_sec', 0):.2f} seconds"
            )
            console.print(f"[bold]Sample Rate:[/bold] {meta.get('sample_rate', 0)} Hz")

            # Create summary table
            table = Table(
                title="Key Features", show_header=True, header_style="bold blue"
            )
            ...
            if quality.get("dynamic_range_db") is not None:
                table.add_row(
                    "", "Dynamic Range", f"{quality['dynamic_range_db']:.1f} dB"
                )
            ...
            if os.path.exists(html_report):
                console.print(
                    f"[green]📊 Detailed HTML report available:[/green] {html_report}"
                )
            ...
            if key_est.get("key"):
                print(
                    f"Estimated Key: {key_est['key']} (confidence: {key_est.get('confidence', 0):.3f})"
                )
...
Examples:
  ...
  python %(prog)s --audio song.mp3 --quick --no-plots
    ...
    input_group.add_argument(
        "--lyrics", type=str, help="Lyrics file (for single audio)"
    )
    input_group.add_argument(
        "--manifest", type=str, help="CSV manifest with audio_path,lyrics_path"
    )

    # Output options
    output_group = parser.add_argument_group("Output Options")
    output_group.add_argument(
        "--out", type=str, default="music_analysis_results", help="Output directory"
    )
    output_group.add_argument(
        "--save-matrices", action="store_true", help="Save large matrices as .npy files"
    )
    output_group.add_argument(
    ...
    process_group.add_argument(
        "--quick", action="store_true", help="Quick analysis (reduced features)"
    )
    ...
    audio_group.add_argument(
        "--use-crepe", action="store_true", help="Use CREPE for pitch estimation"
    )
    audio_group.add_argument(
    ...
    audio_group.add_argument(
        "--force-transcode",
        action="store_true",
        help="Transcode inputs to WAV before analysis",
    )
    audio_group.add_argument(
        "--transcode-dir", type=str, help="Directory to place transcoded WAV files"
    )

    # Interface options
    interface_group = parser.add_argument_group("Interface Options")
    interface_group.add_argument(
        "--verify", action="store_true", help="Show analysis summary after processing"
    )
    interface_group.add_argument(
...
def main():
    ...
    if not args.quiet and HAS_RICH:
        console.print(
            Panel.fit("🎵 Enhanced Music Feature Extraction v4.0", style="bold blue")
        )
    elif not args.quiet:
        ...
        if args.manifest:
            ...
            if result["status"] == "success":
                if not args.quiet:
                    if HAS_RICH:
                        console.print(
                            f"[green]✓ Successfully processed:[/green] {args.audio}"
                        )
                        console.print(
                            f"[blue]Output folder:[/blue] {result['out_folder']}"
                        )
                    else:
