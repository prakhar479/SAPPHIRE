...
query = (
    "SELECT track_id, group_concat(word || ':' || count) FROM lyrics GROUP BY track_id"
)
...
lyrics_df = lyrics_df.rename(
    columns={"group_concat(word || ':' || count)": "lyrics_bow"}
)

print(
    f"Found bag-of-words lyrics for {len(lyrics_df)} tracks in the MusixMatch database."
)
