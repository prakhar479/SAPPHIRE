...
def loudness_curve(y, sr, frame_length=2048, hop_length=512):
    ...
    return {
        "times": times[: len(loudness_curve)].tolist(),
        "loudness_db": loudness_curve,
    }
...
def rhythm_features(y, sr):
    ...
    tempo_bpm = tempo
    beat_times = (
        librosa.frames_to_time(beat_frames, sr=sr) if len(beat_frames) > 0 else []
    )
    ...
    if len(beat_times) >= 1:
        ...
        micro_dev = {
            "median_dev_sec": float(np.median(deviations)) if deviations else 0.0,
...
def structure_segmentation(y, sr, hop_length=512, n_segments=6):
    ...
    if features.shape[0] < n_segments:
        ...
        cluster = AgglomerativeClustering(
            n_clusters=n_segments, metric="cosine", linkage="average"
        )
    ...
    if len(labels) > 0:
        ...
        for i, l in enumerate(labels[1:], start=1):
            if l != cur_label:
                ...
                start_t = times[i]
        segments.append(
            {"label": int(cur_label), "start": float(start_t), "end": float(times[-1])}
        )
...
def repetition_detection_segments(segments):
    ...
    for s in segments:
        h = hashlib.sha1(
            f"{s['label']}-{round(s['end']-s['start'],3)}".encode()
        ).hexdigest()[:8]
