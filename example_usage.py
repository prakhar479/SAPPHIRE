...
def main():
    ...
    for dataset_name, info in datasets_info.items():
        if info['exists']:
            print(f"  ✓ {dataset_name}: {info['audio_files']} audio files, {info['lyrics_files']} lyrics files")
        else:
            print(f"  ✗ {dataset_name}: Not found")
    
    # Run pipeline on available datasets
    available_datasets = [name for name, info in datasets_info.items() if info['exists']]
    ...
    if results['status'] == 'success':
        ...
        if viz_dir.exists():
            ...
            print(f"  ✓ visualizations/ ({len(viz_subdirs)} subdirectories: {', '.join(viz_subdirs)})")
        ...
        if pipeline.mood_classifier.best_model is not None:
            ...
            print("result = pipeline.predict_mood('path/to/audio.mp3', 'path/to/lyrics.txt')")
...
def quick_feature_extraction_example():
    ...
    try:
        ...
        if features_list:
            print(f"Number of features per track: {len(features_list[0]) - 2}")  # -2 for track_id and mood
