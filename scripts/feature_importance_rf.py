...
def prepare_X_y(df: pd.DataFrame, label_col: str):
    ...
    const_cols = [c for c in df.columns if df[c].nunique(dropna=False) <= 1 and c != label_col]
    if const_cols:
    ...
    id_like_numeric = [c for c in numeric_X.columns if 'file_index' in c or c.endswith('.file_index') or c.endswith('.track_id')]
    if id_like_numeric:
        print(f"🔒 Removing numeric identifier-like columns from features: {id_like_numeric}")
        numeric_X = numeric_X.drop(columns=id_like_numeric)

    if numeric_X.shape[1] == 0:
        raise RuntimeError("No numeric feature columns found after filtering. Check your features file.")

    return numeric_X, y


def main():
    parser = argparse.ArgumentParser(description='RandomForest feature importance for mood classification')
    parser.add_argument('--features', '-f', required=True, help='Path to features parquet/csv')
    parser.add_argument('--label', '-l', default='mood_cluster', help='Label column name')
    parser.add_argument('--top', '-t', type=int, default=30, help='Number of top features to show/save')
    parser.add_argument('--output', '-o', default='output/feature_importance', help='Output directory')
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, y_enc, test_size=0.2, random_state=args.random_state, stratify=y_enc if len(np.unique(y_enc))>1 else None
    )
